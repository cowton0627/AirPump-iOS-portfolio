...
- 為圖示導覽、影音播放、警示框與核心裝置控制提供 VoiceOver 支援，動態同步 BLE 連線、播放、擠乳與紀錄分頁選取狀態，互動圖示至少保留 44×44pt 觸控範圍
...
專案包含 `Breast PumpTests` 與 `Breast PumpUITests` targets。前者涵蓋 Demo Mode、Realm record mapping、紀錄統計、版面與 VoiceOver 狀態；後者透過 XCUITest 實際啟動 App，驗證主 tab 導覽、Demo Mode、紀錄主流程切換今日／歷史／分析、偏好設定的提示音與通知開關、影音頁下載控制與類型 popover 三種選項、討論區提示關閉與重新進入時的提示恢復，以及無 BLE 時操作頁控制項仍可呈現。UI 測試使用穩定的 accessibility identifiers，無需 BLE 實體硬體。
