...
專案包含 `Breast PumpTests` 與 `Breast PumpUITests` targets。前者涵蓋 Demo Mode、Realm record mapping、紀錄統計、版面與 VoiceOver 狀態；後者透過 XCUITest 實際啟動 App，驗證紀錄主流程切換今日／歷史／分析，以及偏好設定通知開關。UI 測試使用穩定的 accessibility identifiers，無需 BLE 實體硬體。
