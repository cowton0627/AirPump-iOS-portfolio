...
- 完整 suite 共 14 個 unit/integration tests 與 8 個 XCUITests，0 failures。
- `Breast PumpUITests` 透過 `AIRPUMP_START_TAB=1` 從紀錄頁啟動，使用 `tab.records` 與 `records.today/history/analysis` identifiers 驗證實際點擊與選取狀態；另有流程切換 `preference.beep`／`preference.notify`／`作品集示範模式`、從 `tab.video` 開啟影音類型 popover 選取「影片」，以及從 `tab.discussion` 關閉並重新進入時恢復「敬請期待」提示。
